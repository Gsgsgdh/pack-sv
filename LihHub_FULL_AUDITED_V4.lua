...
pcall(function()
    if v484 and v484.AddSection then
        v484:AddSection("Lih Hub")
    end
end)
...
v485:AddDropdown({
    ...
    Callback = function(p36)
        ...
        elseif p36 == "Bigger" then
            vu32:SetUIScale(1.30)  -- menor número = maior UI
        else
            vu32:SetUIScale(1.00)
        end
    end
...
-- ============================================================
-- LIH HUB RGB TITLE (LOW-OVERHEAD)
-- Find the title once, then only update its color.
-- ============================================================
task.spawn(function()
    task.wait(0.5)
    ...
    pcall(function()
        ...
        RunService.RenderStepped:Connect(function()
            local label = findTitle()
            if label then
                label.TextColor3 = Color3.fromHSV((os.clock() * 0.12) % 1, 0.9, 1)
            end
        end)
    end)
end)
