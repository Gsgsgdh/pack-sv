...
-- ============================================================
-- LIH HUB UI BOOTSTRAP
-- Create the window before the large feature section so an
-- unrelated feature error cannot prevent the menu from appearing.
-- ============================================================

-- ============================================================
-- LIH HUB | CUSTOM UI REDESIGN
-- Replaces the old library UI with a self-contained interface
-- inspired by the supplied cyberpunk screenshot.
-- Existing feature callbacks are kept; this layer only changes UI.
-- ============================================================
...
local GUI_NAME = "LihHub_CustomUI"
pcall(function()
...
local function rgb(t)
    return Color3.fromHSV((t % 1), 0.85, 1)
end

local COLORS = {
    bg = Color3.fromRGB(5, 7, 12),
    panel = Color3.fromRGB(10, 12, 20),
    card = Color3.fromRGB(13, 16, 26),
    row = Color3.fromRGB(18, 20, 30),
    text = Color3.fromRGB(245, 245, 250),
    sub = Color3.fromRGB(160, 165, 180),
...
root.Size = UDim2.new(0, 1080, 0, 650)
...
gradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(9, 7, 17)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(6, 12, 22)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(16, 7, 22)),
})
...
local logo = label(header, "LIH HUB", 32, COLORS.text, Enum.Font.GothamBlack)
logo.Position = UDim2.new(0, 28, 0, 10)
logo.Size = UDim2.new(0, 190, 0, 34)

local subtitle = label(header, "BLOX FRUITS", 12, COLORS.purple, Enum.Font.GothamBold)
subtitle.Position = UDim2.new(0, 30, 0, 44)
subtitle.Size = UDim2.new(0, 150, 0, 18)

local edition = label(header, "RGB EDITION • PREMIUM", 12, COLORS.sub, Enum.Font.Gotham)
edition.Position = UDim2.new(0, 190, 0, 35)
edition.Size = UDim2.new(0, 210, 0, 20)
...
local sidebar = new("Frame", {
    ...
    Size = UDim2.new(0, 235, 1, 0),
    BorderSizePixel = 0,
}, body)
new("UICorner", {CornerRadius = UDim.new(0, 14)}, sidebar)

local sidePad = new("UIPadding", {
...
new("UIListLayout", {Padding = UDim.new(0, 7), SortOrder = Enum.SortOrder.LayoutOrder}, nav)
...
new("UICorner", {CornerRadius = UDim.new(0, 12)}, profile)
new("UIStroke", {Color = COLORS.purple, Transparency = 0.45}, profile)
local profileName = label(profile, tostring(LocalPlayer and LocalPlayer.Name or "Player"), 15, COLORS.text, Enum.Font.GothamBold)
profileName.Position = UDim2.new(0, 14, 0, 12)
profileName.Size = UDim2.new(1, -28, 0, 22)
local profileRole = label(profile, "Premium User", 11, COLORS.purple, Enum.Font.Gotham)
profileRole.Position = UDim2.new(0, 14, 0, 37)
profileRole.Size = UDim2.new(1, -28, 0, 18)

local content = new("Frame", {
    BackgroundTransparency = 1,
    Position = UDim2.new(0, 247, 0, 0),
    Size = UDim2.new(1, -247, 1, 0),
}, body)
...
local function makeTab(title, icon)
    ...
    local bstroke = new("UIStroke", {Transparency = 1, Thickness = 1}, btn)
    ...
    local function addHeader(text)
        ...
        local l = label(f, string.upper(text), 17, COLORS.text, Enum.Font.GothamBold)
    ...
    function tab:AddParagraph(title, desc)
        ...
        new("UIStroke", {Color = COLORS.purple, Transparency = 0.78}, card)
        local a = label(card, tostring(title or ""), 14, COLORS.text, Enum.Font.GothamBold)
        a.Position = UDim2.new(0, 14, 0, 8)
        a.Size = UDim2.new(1, -28, 0, 22)
        local b = label(card, tostring(desc or ""), 11, COLORS.sub, Enum.Font.Gotham)
        b.Position = UDim2.new(0, 14, 0, 31)
        b.Size = UDim2.new(1, -28, 0, 28)
        b.TextWrapped = true
        local obj = {}
        function obj:SetContent(v) b.Text = tostring(v) end
        function obj:SetDesc(v) b.Text = tostring(v) end
        return obj
    end
    ...
    function tab:AddToggle(cfg)
        ...
        if state == nil then state = cfg.Value end
        state = state == true
        ...
        local titleLabel = label(row, tostring(name), 13, COLORS.text, Enum.Font.GothamMedium)
        titleLabel.Position = UDim2.new(0, 14, 0, 7)
        titleLabel.Size = UDim2.new(1, -90, 0, 20)
        local descLabel = label(row, tostring(desc), 10, COLORS.sub, Enum.Font.Gotham)
        ...
        local function render()
            if state then
                track.BackgroundColor3 = COLORS.purple
                knob.Position = UDim2.new(1, -20, 0, 2)
            else
                track.BackgroundColor3 = Color3.fromRGB(40, 43, 53)
        ...
        row.MouseButton1Click:Connect(function()
            ...
            if type(cfg.Callback) == "function" then
                local ok, err = xpcall(function() cfg.Callback(state) end, debug.traceback)
                if not ok then
                    tab._errors += 1
                    warn("[Lih Hub]["..title.."] toggle "..tostring(name)..": "..tostring(err))
                end
            end
        end)

        return {Set = function(_, v) state = v == true; render() end}
    end
    ...
    function tab:AddDropdown(cfg)
        ...
        local l = label(box, tostring(name), 12, COLORS.sub, Enum.Font.Gotham)
        ...
        local select = new("TextButton", {
            ...
            Position = UDim2.new(0.5, 4, 0, 12),
            BorderSizePixel = 0,
            AutoButtonColor = false,
        }, box)
        new("UICorner", {CornerRadius = UDim.new(0, 8)}, select)
        ...
        select.MouseButton1Click:Connect(function()
            ...
            list = new("ScrollingFrame", {
                ...
                Size = UDim2.new(0, select.AbsoluteSize.X, 0, math.min(190, math.max(40, #options * 30))),
                Position = UDim2.new(0, select.AbsolutePosition.X - content.AbsolutePosition.X, 0, select.AbsolutePosition.Y - content.AbsolutePosition.Y + select.AbsoluteSize.Y + 3),
            ...
            for _, opt in ipairs(options) do
                ...
                ob.MouseButton1Click:Connect(function()
                    ...
                    closeList()
                    if type(cfg.Callback) == "function" then
                        local ok, err = xpcall(function() cfg.Callback(opt) end, debug.traceback)
                        if not ok then
                            tab._errors += 1
                            warn("[Lih Hub]["..title.."] dropdown "..tostring(name)..": "..tostring(err))
                        end
                    end
                end)
            end
        end)
        return {Set = function(_, v) selected=v; select.Text=tostring(v) end}
    end
    ...
    function tab:AddButton(cfg)
        ...
        new("UIStroke", {Color=COLORS.cyan, Transparency=0.8}, b)
        b.MouseButton1Click:Connect(function()
            if type(cfg.Callback) == "function" then
                local ok, err = xpcall(cfg.Callback, debug.traceback)
                if not ok then
                    tab._errors += 1
                    warn("[Lih Hub]["..title.."] button "..tostring(name)..": "..tostring(err))
                end
            end
        end)
        return b
    ...
    function tab:AddSlider(cfg)
        ...
        new("UICorner",{CornerRadius=UDim.new(0,10)},holder)
        local l=label(holder,tostring(name),12,COLORS.text,Enum.Font.GothamMedium)
        l.Position=UDim2.new(0,14,0,7); l.Size=UDim2.new(1,-28,0,18)
        local bar=new("Frame",{BackgroundColor3=Color3.fromRGB(38,40,50),Position=UDim2.new(0,14,0,37),Size=UDim2.new(1,-28,0,6),BorderSizePixel=0},holder)
        new("UICorner",{CornerRadius=UDim.new(1,0)},bar)
        local fill=new("Frame",{BackgroundColor3=COLORS.purple,Size=UDim2.new((value-min)/(max-min),0,1,0),BorderSizePixel=0},bar)
        new("UICorner",{CornerRadius=UDim.new(1,0)},fill)
        local knob=new("TextButton",{BackgroundTransparency=1,Text="",Size=UDim2.new(1,0,1,0)},bar)
        local function setFromX(x)
            local pct=math.clamp((x-bar.AbsolutePosition.X)/bar.AbsoluteSize.X,0,1)
            value=math.floor(min+(max-min)*pct+0.5)
            fill.Size=UDim2.new(pct,0,1,0)
            if type(cfg.Callback)=="function" then pcall(cfg.Callback,value) end
        end
        knob.MouseButton1Down:Connect(function()
            ...
            local ended
            ended=UserInputService.InputEnded:Connect(function(inp)
                if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then conn:Disconnect(); ended:Disconnect() end
            end)
        end)
        return {Set=function(_,v) value=v; fill.Size=UDim2.new(math.clamp((v-min)/(max-min),0,1),0,1,0) end}
    end

    function tab:AddDiscordInvite(cfg)
        return tab:AddButton({
            Name = "DISCORD  •  " .. tostring((cfg and cfg.Name) or "Community"),
            Callback = function()
                if setclipboard and cfg and cfg.Invite then
                    pcall(setclipboard, cfg.Invite)
                end
            end
        })
    end
    ...
    btn.MouseButton1Click:Connect(function()
        if currentTab then
            ...
            currentTab._button.BackgroundColor3 = Color3.fromRGB(15,17,26)
        end
        currentTab = tab
        tab._page.Visible = true
        tab._button.BackgroundColor3 = Color3.fromRGB(40,25,65)
    end)
...
local v466 = {}
function v466:MakeTab(info)
    local title = type(info)=="table" and (info[1] or "Tab") or tostring(info)
    local icon = type(info)=="table" and info[2] or ""
    return makeTab(title, icon)
end
function v466:SelectTab(tab)
    tab = tab and (tab.__tab or tab)
    if tab and tab._button then tab._button:Activate() end
    if tab then
        if currentTab then currentTab._page.Visible=false; currentTab._button.BackgroundColor3=Color3.fromRGB(15,17,26) end
        currentTab=tab
        tab._page.Visible=true
        tab._button.BackgroundColor3=Color3.fromRGB(40,25,65)
    end
end
function v466:Notify(cfg)
    local n = new("Frame", {BackgroundColor3=COLORS.panel,Size=UDim2.new(0,340,0,64),Position=UDim2.new(1,-360,1,-84),BorderSizePixel=0,ZIndex=300},gui)
    new("UICorner",{CornerRadius=UDim.new(0,12)},n)
    new("UIStroke",{Color=COLORS.purple,Transparency=0.2},n)
    local t=label(n,tostring((cfg and cfg.Title) or "Lih hub"),14,COLORS.text,Enum.Font.GothamBold); t.Position=UDim2.new(0,14,0,8); t.Size=UDim2.new(1,-28,0,20)
    local c=label(n,tostring((cfg and cfg.Content) or ""),11,COLORS.sub,Enum.Font.Gotham); c.Position=UDim2.new(0,14,0,30); c.Size=UDim2.new(1,-28,0,25)
    task.delay((cfg and cfg.Time) or 3,function() if n then n:Destroy() end end)
end
function v466:SetUIScale(v) scale.Scale = math.clamp(tonumber(v) or 1,0.6,1.6) end

-- SafeTab proxy is defined below to preserve the existing feature code API.
local function SafeTab(tab, title)
    local proxy = {__tab=tab,__title=title,__errors=0}
    return setmetatable(proxy,{
        __index=function(self,key)
            local value=self.__tab[key]
            if type(value)=="function" then
                return function(_, ...)
                    local args=table.pack(...)
                    local ok,res=xpcall(function() return value(self.__tab,table.unpack(args,1,args.n)) end,debug.traceback)
                    if not ok then
                        self.__errors+=1
                        warn("[Lih Hub]["..title.."] "..tostring(key).." failed:\n"..tostring(res))
                        return nil
                    end
                    return res
                end
        ...
        __newindex=function(self,key,value) self.__tab[key]=value end
    })
end

local v484 = SafeTab(v466:MakeTab({"Home","home"}),"Home")
local v485 = SafeTab(v466:MakeTab({"Farming","leaf"}),"Farming")
local v487 = SafeTab(v466:MakeTab({"Quests & Items","swords"}),"Quests & Items")
local v486 = SafeTab(v466:MakeTab({"Fishing","fish"}),"Fishing")
local v489 = SafeTab(v466:MakeTab({"Sea Events","waves"}),"Sea Events")
local v491 = SafeTab(v466:MakeTab({"Raids & Fruits","skull"}),"Raids & Fruits")
local v497 = SafeTab(v466:MakeTab({"Stats","bar"}),"Stats")
local v493 = SafeTab(v466:MakeTab({"Teleport","pin"}),"Teleport")
local v499 = SafeTab(v466:MakeTab({"Status","pulse"}),"Status")
local v494 = SafeTab(v466:MakeTab({"Visuals","eye"}),"Visuals")
local v495 = SafeTab(v466:MakeTab({"Shop","cart"}),"Shop")
local v496 = SafeTab(v466:MakeTab({"Settings","gear"}),"Settings")

-- Dashboard cards matching the supplied reference style.
do
    v484:AddSection("Overview")
    local dash = Instance.new("Frame")
    dash.BackgroundTransparency = 1
    dash.Size = UDim2.new(1,0,0,86)
    dash.Parent = v484.__tab._page
    local grid = Instance.new("UIGridLayout")
    grid.CellSize = UDim2.new(0.19,0,1,0)
    grid.CellPadding = UDim2.new(0.012,0,0,0)
    grid.Parent = dash
    local cards = {
        {"LEVEL","--",COLORS.purple},
        {"BELI","--",COLORS.green},
        {"FRAGMENTS","--",COLORS.cyan},
        {"RACE","--",COLORS.yellow},
        {"SEA","--",COLORS.purple},
    }
    for _,d in ipairs(cards) do
        local c=Instance.new("Frame"); c.BackgroundColor3=COLORS.card; c.BorderSizePixel=0; c.Parent=dash
        Instance.new("UICorner",c).CornerRadius=UDim.new(0,10)
        local st=Instance.new("UIStroke",c); st.Color=d[3]; st.Transparency=0.55
        local a=label(c,d[1],10,COLORS.sub,Enum.Font.GothamBold); a.Position=UDim2.new(0,10,0,8); a.Size=UDim2.new(1,-20,0,18)
        local b=label(c,d[2],18,d[3],Enum.Font.GothamBold); b.Position=UDim2.new(0,10,0,30); b.Size=UDim2.new(1,-20,0,34)
        if d[1]=="LEVEL" then
            task.spawn(function()
                while task.wait(1) do
                    pcall(function()
                        b.Text=tostring(LocalPlayer.Data.Level.Value)
                    end)
                end
            end)
        elseif d[1]=="BELI" then
            task.spawn(function()
                while task.wait(1) do
                    pcall(function()
                        b.Text=tostring(LocalPlayer.Data.Beli.Value)
                    end)
                end
            end)
        elseif d[1]=="FRAGMENTS" then
            task.spawn(function()
                while task.wait(1) do
                    pcall(function()
                        b.Text=tostring(LocalPlayer.Data.Fragments.Value)
                    end)
                end
            end)
        elseif d[1]=="RACE" then
            task.spawn(function()
                while task.wait(2) do
                    pcall(function()
                        b.Text=tostring(LocalPlayer.Data.Race.Value)
                    end)
                end
            end)
        end
...
-- Select the Farming page by default, as in the supplied screenshot.
pcall(function() v466:SelectTab(v485) end)

-- RGB border/title animation.
task.spawn(function()
    while gui.Parent do
        local c = rgb(os.clock()*0.12)
        stroke.Color = c
        logo.TextColor3 = c
        task.wait(0.04)
    end
...
minimize.MouseButton1Click:Connect(function()
    ...
    root.Size=minimized and UDim2.new(0,1080,0,82) or UDim2.new(0,1080,0,650)
end)
close.MouseButton1Click:Connect(function() root.Visible=false end)

-- Compatibility object used by the existing UI-scale feature.
local vu32 = v466
...
-- Optional hooks: failure here must never stop the UI.
pcall(function()
    local Effect = game:GetService("ReplicatedStorage"):FindFirstChild("Effect")
    local Container = Effect and Effect:FindFirstChild("Container")
    local Death = Container and Container:FindFirstChild("Death")
    if Death and type(hookfunction) == "function" then
...
pcall(function()
    local Effect = game:GetService("ReplicatedStorage"):FindFirstChild("Effect")
    local Container = Effect and Effect:FindFirstChild("Container")
    local Respawn = Container and Container:FindFirstChild("Respawn")
    if Respawn and type(hookfunction) == "function" then
...
v485:AddDropdown({
    ...
    Callback = function(p36)
        if p36 == "Small" then
            vu32:SetUIScale(0.75)
        elseif p36 == "Large" then
            vu32:SetUIScale(1.00)
        elseif p36 == "Bigger" then
            vu32:SetUIScale(1.30)
        else
            vu32:SetUIScale(1.00)
        end
    end
...
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            ...
            v548:SetContent("Status: " .. v549 .. " Eye(s)" .. (not (v549 ~= 4) and "       " or ""))
        end)
    end
end)
v485:AddToggle({
...
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            ...
            v589:SetContent("You Have: " .. tostring(v590) .. " Bones")
        end)
    end
end)
v485:AddToggle({
    ...
    while task.wait(1) do
        pcall(function()
            ...
            if string.len(v607) == 88 then
                v606:SetContent("Killed : " .. string.sub(v607, 39, 41) .. " / 500")
            elseif string.len(v607) ~= 87 then
                if string.len(v607) == 86 then
                    v606:SetContent("Killed : " .. string.sub(v607, 39, 39) .. " / 500")
                else
                    v606:SetContent("Prince King Spawned    ")
                end
            else
                v606:SetContent("Killed : " .. string.sub(v607, 39, 40) .. " / 500")
            end
        end)
    end
end)
    ...
    while task.wait(1) do
        pcall(function()
            if _G.SelectBoss and (game:GetService("ReplicatedStorage"):FindFirstChild(_G.SelectBoss) or game:GetService("Workspace").Enemies:FindFirstChild(_G.SelectBoss)) then
                v657:SetContent("Status: Boss Spawn    ")
            else
                v657:SetContent("Status: Boss Not Spawn    ")
            end
        end)
    end
end)
    ...
    while task.wait(1) do
        pcall(function()
            if game:GetService("Workspace").Map:FindFirstChild("PrehistoricIsland") then
                v850:SetContent("Prehistoric Island Spawning    ")
            else
                v850:SetContent("Prehistoric Island Not Spawn    ")
            end
        end)
    end
end)
    ...
    while task.wait(1) do
        pcall(function()
            if game:GetService("Workspace").Map:FindFirstChild("KitsuneIsland") then
                v939:SetContent("Kitsune Island Spawning    ")
            else
                v939:SetContent("Kitsune Island Not Spawn    ")
            end
        end)
    end
end)
    ...
    while task.wait(1) do
        pcall(function()
            if not game.Workspace._WorldOrigin.Locations:FindFirstChild("Mirage Island") then
                v984:SetContent("Mirage Island Not Spawn    ")
            else
                v984:SetContent("Mirage Island Spawning    ")
            end
        end)
    end
end)
...
-- ============================================================
-- LIH HUB INITIALIZATION STATUS
-- ============================================================
pcall(function()
    if v466 and v466.Notify then
        v466:Notify({
            Title = "Lih hub",
            Content = "UI loaded. Feature registration completed.",
            Time = 3
        })
    end
end)
...
-- ============================================================
-- LIH HUB VISUAL THEME
-- RGB title accent + subtle RGB border. Visual failures are isolated.
-- ============================================================
task.spawn(function()
    task.wait(0.75)
    pcall(function()
        ...
        local function scan()
            if titleLabel and titleLabel.Parent and windowFrame and windowFrame.Parent then return end
            for _, root in ipairs(roots) do
                for _, obj in ipairs(root:GetDescendants()) do
                    if obj:IsA("TextLabel") and obj.Text == "Lih hub | Blox Fruits" then
                        titleLabel = obj
                    elseif obj:IsA("Frame") and obj.Name == "Window" then
                        windowFrame = obj
                    end
                end
            end
        ...
        connection = RunService.RenderStepped:Connect(function()
            if not titleLabel or not titleLabel.Parent or not windowFrame or not windowFrame.Parent then
                scan()
            end
            local c = Color3.fromHSV((os.clock() * 0.10) % 1, 0.85, 1)
            if titleLabel then titleLabel.TextColor3 = c end
            if windowFrame then
                local stroke = windowFrame:FindFirstChild("LihHubRGBStroke")
                if not stroke then
                    stroke = Instance.new("UIStroke")
                    stroke.Name = "LihHubRGBStroke"
                    stroke.Thickness = 1.5
