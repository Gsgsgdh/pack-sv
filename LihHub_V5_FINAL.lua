...
-- Lih hub global configuration
getgenv().LihHubConfig = getgenv().LihHubConfig or {
    FlySpeed = 250,
    TweenSpeed = 250,
}
...
do
    local LIB_URL = "https://raw.githubusercontent.com/tlredz/Library/refs/heads/main/redz-V5-remake/main.luau"
    local okHttp, sourceOrErr = pcall(function()
        return game:HttpGet(LIB_URL)
    end)

    if not okHttp or type(sourceOrErr) ~= "string" or #sourceOrErr == 0 then
        warn("[Lih Hub] UI library download failed:", tostring(sourceOrErr))
        return
    end

    local loader, compileErr = loadstring(sourceOrErr, "LihHub_UI_Library")
    if not loader then
        warn("[Lih Hub] UI library compile failed:", tostring(compileErr))
        return
    end

    local okLoad, libraryOrErr = pcall(loader)
    if not okLoad or type(libraryOrErr) ~= "table" then
        warn("[Lih Hub] UI library load failed:", tostring(libraryOrErr))
        return
    end
    ...
    local okWindow, windowOrErr = pcall(function()
        return vu32:MakeWindow({
            ...
            ScriptFolder = "Redz | redz lib v5.lua"
        })
    end)

    if not okWindow or type(windowOrErr) ~= "table" then
        warn("[Lih Hub] Window creation failed:", tostring(windowOrErr))
        return
    end
...
-- Original Lih hub minimize button/icon.
pcall(function()
    v466:AddMinimizeButton({
        Button = { Image = "rbxassetid://84204201817869", BackgroundTransparency = 0 },
...
-- ============================================================
-- LIH HUB TABS: CREATE THESE IMMEDIATELY AFTER THE WINDOW.
-- This prevents a feature error later in the script from hiding
-- the menu and all of its navigation.
-- ============================================================
-- ============================================================
-- LIH HUB - NEW SAFE NAVIGATION LAYER
-- Every feature tab is proxied so one bad control cannot stop
-- the remaining controls from being registered.
-- ============================================================
            ...
            if type(value) == "function" then
                return function(_, ...)
                    ...
                    local ok, result = xpcall(function()
                        return value(self.__tab, table.unpack(args, 1, args.n))
                    end, debug.traceback)

                    if not ok then
                        self.__errors += 1
                        warn(string.format("[Lih Hub][%s] %s failed:\n%s", self.__title, key, tostring(result)))
                        return nil
                    end
...
local v485 = SafeTab(v466:MakeTab({"Farming", "home"}), "Farming")
local v487 = SafeTab(v466:MakeTab({"Quests & Items", "swords"}), "Quests & Items")
local v486 = SafeTab(v466:MakeTab({"Fishing", "rbxassetid://127664059821666"}), "Fishing")
local v489 = SafeTab(v466:MakeTab({"Sea Events", "waves"}), "Sea Events")
local v491 = SafeTab(v466:MakeTab({"Raids & Fruits", "cherry"}), "Raids & Fruits")
local v497 = SafeTab(v466:MakeTab({"Stats", "Signal"}), "Stats")
local v493 = SafeTab(v466:MakeTab({"Teleport", "locate"}), "Teleport")
local v499 = SafeTab(v466:MakeTab({"Status", "Scroll"}), "Status")
local v494 = SafeTab(v466:MakeTab({"Visuals", "user"}), "Visuals")
local v495 = SafeTab(v466:MakeTab({"Shop", "shoppingCart"}), "Shop")
local v496 = SafeTab(v466:MakeTab({"Settings", "settings"}), "Settings")

pcall(function() v466:SelectTab(v485) end)

-- ============================================================
-- LIH HUB GUI ONLY: BACKGROUND
-- This section changes only the GUI; existing feature logic is
-- intentionally left untouched.
-- ============================================================
local LihHubBackground = {
    Enabled = false,
    Image = "",
    Transparency = 0.15,
...
local function ApplyLihHubBackground()
    pcall(function()
        local candidates = {
            v466.Main,
            v466.MainFrame,
        ...
        if not main then
            -- Fall back to the first suitable GUI container owned by the library.
            local playerGui = Plr:FindFirstChildOfClass("PlayerGui")
            if playerGui then
                for _, obj in ipairs(playerGui:GetDescendants()) do
                    if obj:IsA("Frame") and obj.AbsoluteSize.X > 400 and obj.AbsoluteSize.Y > 250 then
                        main = obj
                        break
                    end
                end
        ...
        local bg = main:FindFirstChild("LihHubBackground")
        if not bg then
            bg = Instance.new("ImageLabel")
            bg.Name = "LihHubBackground"
            bg.BackgroundTransparency = 1
            bg.BorderSizePixel = 0
            bg.Size = UDim2.fromScale(1, 1)
            bg.Position = UDim2.fromScale(0, 0)
            bg.ScaleType = Enum.ScaleType.Crop
            bg.ZIndex = math.max(0, main.ZIndex)
            bg.Parent = main
        end

        bg.Visible = LihHubBackground.Enabled and LihHubBackground.Image ~= ""
        bg.Image = LihHubBackground.Image
        bg.ImageTransparency = LihHubBackground.Transparency
        bg.ImageColor3 = LihHubBackground.Color
    end)
end

getgenv().LihHubBackground = LihHubBackground
getgenv().SetLihHubBackground = function(image, transparency, color)
    LihHubBackground.Image = tostring(image or "")
    LihHubBackground.Enabled = LihHubBackground.Image ~= ""
    if transparency ~= nil then
        LihHubBackground.Transparency = math.clamp(tonumber(transparency) or 0.15, 0, 1)
    end
    if typeof(color) == "Color3" then
        LihHubBackground.Color = color
    end
    ApplyLihHubBackground()
end

pcall(function()
    v496:AddSection("Lih hub Background")

    v496:AddToggle({
        Name = "Enable GUI Background",
        Default = false,
        Callback = function(value)
            LihHubBackground.Enabled = value == true and LihHubBackground.Image ~= ""
            ApplyLihHubBackground()
        end
    })

    -- Uses the library's textbox API when available.
    if type(v496.AddTextbox) == "function" then
        v496:AddTextbox({
            Name = "Background Image ID / URL",
            Default = "",
            TextDisappear = false,
            Callback = function(value)
                LihHubBackground.Image = tostring(value or "")
                LihHubBackground.Enabled = LihHubBackground.Image ~= ""
                ApplyLihHubBackground()
            end
        })
    ...
    v496:AddSlider({
        ...
        Default = 15,
        Callback = function(value)
            LihHubBackground.Transparency = math.clamp((tonumber(value) or 15) / 100, 0, 1)
            ApplyLihHubBackground()
        end
    })

    v496:AddButton({
        Title = "Remove GUI Background",
        Callback = function()
            LihHubBackground.Enabled = false
            LihHubBackground.Image = ""
            ApplyLihHubBackground()
        end
    })
end)

task.defer(ApplyLihHubBackground)

pcall(function()
    v496:AddSlider({
        Name = "Global Movement Speed",
        Min = 50,
        Max = 500,
        Default = getgenv().LihHubConfig.FlySpeed,
        Callback = function(value)
            local n = tonumber(value) or 250
            getgenv().LihHubConfig.FlySpeed = n
            getgenv().LihHubConfig.TweenSpeed = n
        end
    })
end)
...
pcall(function() v484:AddDiscordInvite({
...
pcall(function()
    v484:AddSection("Lih Hub Dashboard")
    v484:AddParagraph("Lih hub | Blox Fruits", "New interface • Safe feature registration • RGB theme")
    v484:AddParagraph("UI Status", "The navigation is loaded first. Feature errors are isolated per control so one broken option does not hide the rest of the menu.")
    v484:AddParagraph("Version", "Lih Hub UI v5 • Premium RGB")
    v484:AddButton({
        Name = "Refresh UI Status",
        Callback = function()
            local errors = {}
            for name, tab in pairs({
                Home=v484, Farming=v485, Quests=v487, Fishing=v486,
                Sea=v489, Raids=v491, Stats=v497, Teleport=v493,
                Status=v499, Visuals=v494, Shop=v495, Settings=v496
            }) do
                if tab.__errors and tab.__errors > 0 then
                    table.insert(errors, name .. ": " .. tostring(tab.__errors))
                end
            end
            if #errors == 0 then
                v466:Notify({Title="Lih hub", Content="All UI controls registered without captured UI errors.", Time=3})
            else
                v466:Notify({Title="Lih hub", Content="Captured UI errors: " .. table.concat(errors, " | "), Time=5})
            end
        end
    })
end)
...
function TPB(v432)
    ...
    local v434 = TweenInfo.new((game:GetService("Workspace").Boats.PirateBrigade.VehicleSeat.CFrame.Position - v432.Position).Magnitude / getgenv().LihHubConfig.TweenSpeed, Enum.EasingStyle.Linear)
...
local function TweenTo(cf)
    ...
    local speed = getgenv().LihHubConfig.FlySpeed
    local t = dist / speed
...
function TweenToPosition(v566)
    ...
    if not v568 then
        ...
        local v570 = (v568.Position - v566.Position).Magnitude / getgenv().LihHubConfig.TweenSpeed
